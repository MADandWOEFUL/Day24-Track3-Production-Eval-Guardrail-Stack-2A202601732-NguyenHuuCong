...
def _enrich_single_item(chunk: dict, methods: list[str]) -> EnrichedChunk:
    text = chunk["text"]
...
def enrich_chunks(
    ...
    if len(chunks) <= 2:
        return [_enrich_single_item(c, methods) for c in chunks]

    with ThreadPoolExecutor(max_workers=8) as executor:
        enriched = list(executor.map(lambda c: _enrich_single_item(c, methods), chunks))
