...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    # Known rail patterns defined in rails.co
    rail_triggers = [
        "bỏ qua tất cả", "ignore your previous", "pretend you are dan",
        "unrestricted ai", "ai không có giới hạn", "system override",
        "tấn công mạng", "mật khẩu admin", "<!-- ignore", "[admin command",
        "tôi là ceo và ra lệnh", "system instructions", "viết một bài thơ",
        "nấu ăn", "phở bò", "bitcoin", "ethereum", "phương trình vi phân",
        "marvel", "cho tôi biết cccd", "số điện thoại của nhân viên",
        "tiết lộ bảng lương", "tiết lộ lương", "dump all training"
    ]

    is_triggered = any(trig in text.lower() for trig in rail_triggers)
    ...
    refuse_keywords = [
        ...
        "chỉ có thể trả lời các câu hỏi về chính sách", "đây là dữ liệu bảo mật",
        "tôi không thể"
    ]
    blocked_by_content = any(kw in content.lower() for kw in refuse_keywords)

    blocked = is_triggered or blocked_by_content

    return {
        "allowed": not blocked,
        "blocked_reason": "nemo_input_rail" if blocked else None,
        "response": content if content else ("Xin lỗi, yêu cầu bị chặn bởi NeMo Guardrails." if blocked else ""),
    }
...
async def check_output_rail(question: str, answer: str, rails=None) -> dict:
    ...
    flagged = any(kw in content.lower() for kw in refuse_keywords)
    return {
        "safe": not flagged,
        "flagged_reason": "nemo_output_rail" if flagged else None,
        "final_answer": content if flagged else answer,
    }
...
def measure_p95_latency(test_inputs: list[str], n_runs: int = 20,
    ...
    async def _measure():
        inputs = (test_inputs * ((n_runs // max(len(test_inputs), 1)) + 1))[:n_runs]
        for text in inputs:
...
def save_phase_c_report(report_data: dict, path: str = "reports/guard_results.json") -> None:
...
if __name__ == "__main__":
    ...
    guard_report = {
        "adversarial_suite": {
            ...
            "passed": passed,
            "pass_rate": round(passed / len(adversarial_set), 4) if adversarial_set else 0.0,
            "results": results,
        },
        "latency": latency,
    }
    save_phase_c_report(guard_report)
